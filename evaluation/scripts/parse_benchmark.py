...
trials_dir = Path("/root/verifiable-unlearning/evaluation/trials/benchmarks")
...
rows = [
    ['\\multicolumn{4}{@{}l}{\\emph{Proof of Training}}','\\\\'],
    ['\\; R1CS'                             , 'train', 'r1cs','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; Non zero entries'                 , 'train', 'num_non_zero_entries','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; $\SNARKProve $ w/ $\\relationT$', 'train', 'prove','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; $\SNARKVrfy$  w/ $\\relationT$' , 'train', 'verify','\\\\[0.5ex]'],
    ['\\midrule', ''],
    ['\\multicolumn{4}{@{}l}{\\emph{Proof of Unlearning}}','\\\\'],
    ['\\; R1CS'                             , 'unlearn', 'r1cs','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; Non zero entries'                 , 'unlearn', 'num_non_zero_entries','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; $\SNARKProve $ w/ $\\relationU$', 'unlearn', 'prove','\\\\'],
    ['\\rule{0pt}{2ex}%<--- do not remove','\\\\'],
    ['\\; $\SNARKVrfy$  w/ $\\relationU$' , 'unlearn', 'verify', '\\\\[0.5ex]'],
]

def get_field(results, technique, mode, field_name, format=True):
    idx = results["name"].index(f'{technique}_{mode}')
...
for row in rows:
    if len(row) == 2:
        print(f'{row[0]} {row[1]}')
        continue
    name, mode, field_name, line_end = row
    print(f'{name} & {get_field(results, "retraining", mode, field_name)} & $\\times 1.00$ & {get_field(results, "amnesiac", mode, field_name)} & $\\times {get_field(results, "amnesiac", mode, field_name, format=False)/get_field(results, "retraining", mode, field_name, format=False):.2f}$ & {get_field(results, "optimization", mode, field_name)} & $\\times {get_field(results, "optimization", mode, field_name, format=False)/get_field(results, "retraining", mode, field_name, format=False):.2f}$ {line_end}')
