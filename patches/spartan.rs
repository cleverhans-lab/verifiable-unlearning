...
pub fn prove<P: AsRef<Path>>(
    ...
    let (inst, wit, inps, num_cons, num_vars, num_inputs, _num_non_zero_entries) =
...
pub fn r1cs_to_spartan(
    ...
    inputs_map: &HashMap<String, Value>,
) -> (Instance, Assignment, Assignment, usize, usize, usize, usize) {
    ...
    let number_non_zero_entries_a = &m_a.len();
    let number_non_zero_entries_b = &m_b.len();
    let number_non_zero_entries_c = &m_c.len();
    let num_non_zero_entries = *cmp::max(cmp::max(number_non_zero_entries_a, number_non_zero_entries_b), number_non_zero_entries_c);
